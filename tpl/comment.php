...
if (!empty($_POST)) {
	...
	$title = CommentManagement::getTargetTitle($db, $_POST['TABLE'], $_POST['FK']);
	...
	if ($uid > 0) {
		...
		if (array_key_exists("RATING", $_POST) && !empty($_POST["RATING"])) {
			if (($_POST["RATING"] <= 0) || ($_POST["RATING"] > 5)) {
				...
				$articleRated = $db->fetch_atom("SELECT count(*) FROM `comment` WHERE `TABLE`='".mysql_real_escape_string($_POST["TABLE"])."' AND FK='".mysql_real_escape_string($_POST["FK"])."'
		...
		if (empty($err)) {
			#$rating = (array_key_exists("RATING_ENABLED", $_POST) ? $rating : null);
			$id_comment = $commentManagement->addComment((int)$_POST["FK"], $title, trim($_POST['comment']), $uid, $rating);
			if ($id_comment > 0) {
		...
		if (empty($err)) {
			$rating = ($nar_systemsettings['MARKTPLATZ']['ANONYMOUS_RATINGS'] ? $rating : null);
			$id_comment = $commentManagement->addCommentAnonymous((int)$_POST["FK"], $title, trim($_POST['comment']), $_POST['name'], $_POST['email'], $rating);
			if ($id_comment > 0) {
				$ar_comment = $commentManagement->fetchOneByParams(array("ID_COMMENT" => $id_comment));
				if ($ar_comment["IS_CONFIRMED"]) {
					$tpl_content->addvar("confirmed", 1);
	...
	if (!empty($err)) {
		...
		$tpl_content->addvar("FK", $_POST["FK"]);
...
$showRating = $nar_systemsettings['MARKTPLATZ']['ALLOW_COMMENTS_RATED'] 
	&& ($tpl_content->vars['SHOW_RATING'] || $nar_systemsettings['MARKTPLATZ']['GLOBAL_RATINGS']);

$tpl_content->addvar("SHOW_RATING_ANONYMOUS", $nar_systemsettings['MARKTPLATZ']['ANONYMOUS_RATINGS']);
$tpl_content->addvar("SHOW_RATING", $showRating);
$already_rated = false;
if ($uid > 0) {
    $result = $db->fetch1("SELECT MIN(ID_COMMENT) AS ID_COMMENT, count(*) AS CNT FROM `comment` WHERE `TABLE`='".mysql_real_escape_string($tpl_content->vars['TABLE'])."' AND FK='".mysql_real_escape_string($tpl_content->vars['FK'])."'
					AND RATING IS NOT NULL AND FK_USER=".(int)$uid);
    if ($result["CNT"] > 0) {
        $already_rated = true;

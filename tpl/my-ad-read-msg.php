...
if ($id) {
    if(isset($_POST) && $_POST['DO'] == 'REPLY') {
        if($chatUserManagement->existUserForChatId($id, $uid)) {
            ...
            if(isset($_POST['BODY']) && trim($_POST['BODY']) != "") {
                ...
                die(forward($tpl_content->tpl_uri_action("my-ad-read-msg,".$id)."#last"));
            } else {
		...
		if($chatUserManagement->existUserForChatId($id, $uid)) {
            ...
            die(forward($tpl_content->tpl_uri_action("my-ad-msg")));
		}
    ...
    $tpl_content->addlist("liste", $tplChatMessages, "tpl/" . $s_lang . "/my-ad-read-msg.row.htm");

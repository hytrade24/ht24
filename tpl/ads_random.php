...
$cacheFile = $ab_path."cache/marktplatz/random/".$cacheHash.".htm";
...
if (($cacheLifetime <= 0) || ($cacheFileAge === false) || ($cacheFileAge > $cacheLifetime)) {
    ...
    if (!empty($tplNameBase)) {
        $tplFileBase = CacheTemplate::getHeadFile("tpl/".$s_lang."/ads_random.".$tplNameBase.".htm");
    ...
    // Query ad ids
    $queryAdIds = Rest_MarketplaceAds::getQueryByParams($arQueryOptions);
    $queryAdIds->addField("ID_AD_MASTER");
    $adsIds = $queryAdIds->fetchCol();
    
    // Get randomized result
    $ads = array();
    if (!empty($adsIds)) {
    	$queryAds = Rest_MarketplaceAds::getQueryByParams(array("ID_AD_MASTER_IN" => "(".implode(", ", $adsIds).")"));
        $queryAds->addSortField("RANDOM", "ASC");
        $queryAds->setLimit($maxAds);
        Rest_MarketplaceAds::addQueryFieldsByTemplate($queryAds, "ads_random.".$template.".htm");
    	$ads = $queryAds->fetchTable();
        Rest_MarketplaceAds::extendAdDetailsList($ads);
    	#die(var_dump($queryAds->getQueryString(), $ads));
    }
    ...
    $tpl_content->addlist("liste", $ads, "tpl/".$s_lang."/ads_random.".$template.".htm");
    ...
    if(!$_SESSION["USER_IS_ADMIN"]) {
        if ($cacheLifetime > 0) {
            if (!is_dir($ab_path."cache/marktplatz/random")) {
                // Cache Verzeichnis erstellen
                mkdir($ab_path."cache/marktplatz/random", 0777, true);
            }

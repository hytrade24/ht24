...
if ($actionSub == "process") {
    ...
    foreach ($arSelectedAds as $adId => $adTable) {
        AdManagment::DeleteByUser($adId, $uid, $adTable);
    }
    // -----------------------------------
    if ($done) {
        $successUrl = $_SESSION["USER_ADS_ACTION_SUCCESS"];
...
if (array_key_exists("confirm", $_POST)) {
    ...
    die(forward( $tpl_content->tpl_uri_action("my-marktplatz,delete,process") ));
} else {
...
$articleCount = $db->fetch_atom("
  SELECT COUNT(*)
  FROM `ad_master`
  WHERE ID_AD_MASTER IN (".implode(", ", $arSelected).") AND FK_USER=".$uid." AND DELETED=0");

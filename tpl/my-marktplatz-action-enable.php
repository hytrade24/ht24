...
if ($actionSub == "process") {
    ...
    foreach ($arSelectedAds as $adId => $adTable) {
        AdManagment::Enable($adId, $adTable);
    }
    // -----------------------------------
    if ($done) {
        $successUrl = $_SESSION["USER_ADS_ACTION_SUCCESS"];
...
if (array_key_exists("confirm", $_POST)) {
    $successUrl = (!empty($_POST["SUCCESS"]) ? $_POST["SUCCESS"] : $_SERVER["HTTP_REFERER"]);
    ...
    die(forward( $tpl_content->tpl_uri_action("my-marktplatz,enable,process") ));
} else {
...
$articleCountDisabled = $db->fetch_atom("
  SELECT COUNT(*)
  FROM `ad_master`
  WHERE ID_AD_MASTER IN (".implode(", ", $arSelected).") AND FK_USER=".$uid." AND STATUS NOT IN (0,1,3,4,5,7) AND (STAMP_DEACTIVATE<STAMP_END)");
$articleCountTimeout = $db->fetch_atom("
  SELECT COUNT(*)
  FROM `ad_master`
  WHERE ID_AD_MASTER IN (".implode(", ", $arSelected).") AND FK_USER=".$uid." AND STATUS IN (0,2,4) AND (STAMP_DEACTIVATE IS NULL OR STAMP_DEACTIVATE>STAMP_END)");

// Add articles and variables to template
$tpl_content->addvar("count", ($articleCountEnabled + $articleCountDisabled + $articleCountTimeout));
$tpl_content->addvar("countEnabled", $articleCountEnabled);
$tpl_content->addvar("countDisabled", $articleCountDisabled);
$tpl_content->addvar("countTimeout", $articleCountTimeout);
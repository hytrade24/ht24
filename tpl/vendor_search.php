...
/* ###VERSIONSBLOCKINLCUDE### */
 ...
 $subtplConfig = Template_Modules_SubTemplates::create($tpl_content, "vendor_search", "Marktplatz Suchmaske");
 ...
 if ($designStyle != "default") {
     $tpl_content->LoadText("tpl/".$s_lang."/vendor_search.".$designStyle.".htm");
 }
 
 // Cache-Datei prüfen
 $cacheHash = md5( serialize($arSettings) );
 $cacheDir = $ab_path."cache/marktplatz/vendor/search";
 if (!is_dir($cacheDir)) {
 ...
 if (($cacheLifetime <= 0) || ($cacheFileAge === false) || ($cacheFileAge > $cacheLifetime)) {
    ...
    if ((int)$articleTablesChild === 0) {
     ...
     $arBaseGroups = array(
         "general"   => new Template("tpl/".$s_lang."/vendor_search.group.base.htm"),
         "nogroup"   => array(),
         "location"  => new Template("tpl/".$s_lang."/vendor_search.group.location.htm")
     );
     ...
     foreach ($arGroupsOrdered as $groupIndex => $arGroupBase) {
         ...
         $tplGroup = new Template("tpl/".$s_lang."/vendor_search.group.htm");
         $tplGroup->addvars($arGroup, "GROUP_");
         $tplGroup->addlist("fields", $arFields, "tpl/".$s_lang."/vendor_search.field.htm");
         $arGroupsOrdered[$groupIndex]["TEMPLATE"] = $tplGroup->process(false);
     }
     // Insert enabled base templates
     $arBaseList = explode(",", $nar_systemsettings["MARKTPLATZ"]["SEARCH_BASE_GROUPS_VENDOR"]);
         ...
         if (array_key_exists($groupName, $arBaseGroups)) {
             ...
             if (!is_array($arBaseGroups[$groupName])) {
                 ...
                 if ($groupName == "general") {
                     $tplGroup->addlist("fields", $arBaseGroups["nogroup"], "tpl/".$s_lang."/vendor_search.field.htm");
                 }
             }
             if ($tplGroup !== false) {
     ...
     $rowSpanIndex = 0;
     $arRowGroups = array();
     $tplRow = new Template("tpl/".$s_lang."/vendor_search.row.htm");
     ...
     if(!$_SESSION["USER_IS_ADMIN"]) {
         if ($cacheLifetime > 0) {
             if (!is_dir($ab_path . "cache/marktplatz/vendor/search")) {
                 // Cache Verzeichnis erstellen
                 mkdir($ab_path . "cache/marktplatz/vendor/search", 0777, true);
             }
 ...
 if (!empty($search_hash) && ($search_hash != "{SEARCH_HASH}")) {
     ...
     if ( isset($arSearchValues["PLZ"]) ) {
	     $arSearchValues["B_PLZ"] = $arSearchValues["PLZ"];
     }
	 if ( isset($arSearchValues["ORT"]) ) {
		 $arSearchValues["B_ORT"] = $arSearchValues["ORT"];
	 }

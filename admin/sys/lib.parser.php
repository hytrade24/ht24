...
function do_bbcode_color ($action, $attributes, $content, $params, &$node_object) {
    if (isset ($attributes['default'])) {
            return '<font style="color:'.htmlspecialchars ($attributes['default']).'">'.$content.'</font>';
    }
}
...
function do_bbcode_size ($action, $attributes, $content, $params, &$node_object) {
    if (isset ($attributes['default'])) {
            return '<font style="font-size:'.htmlspecialchars ($attributes['default']).'px;">'.$content.'</font>';
    }
}
...
function do_bbcode_code ($action, $attributes, $content, $params, &$node_object) {
    if (!isset ($attributes['default'])) {
            return '<p class="code"><b style="font-family:Arial,Verdana;">
							Code:</b><br>'.nl2br(stdHtmlspecialchars($content)).'</p>';
    }
}

...
/* ###VERSIONSBLOCKINLCUDE### */
  ...
  class TreeCategories {
    ...
    function __construct($table, $root) {
      ...
      $this->table = $table;
      $this->root = $root;
      // Cache fÃ¼r Elemente initialisieren
      $this->cache_nodes = array();
      // Sperrung Ã¼berprÃ¼fen
    ...
    public function tree_create_nestedset($sort = "order") {
	  ...
	  $tmpCachedChildTree = $db->fetch_table("SELECT ID_KAT, ORDER_FIELD, PARENT FROM `".$this->table."`
		   WHERE ROOT=".$root."");

      $cachedChildTree = array();
      foreach ($tmpCachedChildTree as $key => $value) {
		  $cachedChildTree[$value['PARENT']][] = $value;
      }

	  $tmpCachedNodes = $db->fetch_table("SELECT el.*, s.T1, s.V1, s.V2 FROM `".$this->table."` el
	          ...
	          WHERE ROOT=".$this->root."  ORDER BY el.ORDER_FIELD");

		$cachedChildNodes = array();
	  foreach ($tmpCachedNodes as $key => $value) {
		  $cachedChildNodes[$value['ID_'.strtoupper($this->table)]][] = $value;
	  }
      ...
      $nestedset_ar[] = array(
      		...
      		"childs"	=> $this->node_create_nestedset($root, $sort, $cachedChildTree, $cachedChildNodes)
      );
    ...
    public function element_read($id, $bf_lang = false) {
      ...
      $node = $db->fetch1("SELECT el.*, s.T1, s.V1, s.V2 FROM `".$this->table."` el
        LEFT JOIN `string_".$this->table."` s ON s.S_TABLE='".$this->table."' AND s.FK=el.ID_KAT
          AND s.BF_LANG=if(el.BF_LANG_KAT & ".$bf_lang.", ".$bf_lang.", 1 << floor(log(el.BF_LANG_KAT+0.5)/log(2)))
        WHERE ROOT=".$this->root." AND ID_KAT=".$id." ORDER BY el.ORDER_FIELD");
      if ($node) {
      	// Split T1 Field into T1 (Keywords) and META (Meta-Tags)
      	list($node["T1"], $node["META"]) = explode("||||", $node["T1"]);
        // Unserialize options
        $node["OPTIONS"] = unserialize($node["SER_OPTIONS"]);
    ...
    public function element_update($id, $data, $inherit = false) {
      ...
      if ($this->updateid = $db->update($this->table, $datanew)) {
        ...
        if (!$inherit && isset($data["KAT_TABLE"]) &&
          ...
          // Tabelle wurde geÃ¤ndert und soll nicht auf Kind-Elemente Ã¼bertragen werden, zwingen!
          $datanew = array();
    ...
    public function element_move($id, $id_target, $inherit_childs = true) {
      ...
      if ($this->updateid = $db->update($this->table, $new)) {
        unset($this->cache_nodes[$this->updateid]);
        if (!$inherit_childs) {
          // Kind-Elemente eine Ebene aufwÃ¤rts verschieben

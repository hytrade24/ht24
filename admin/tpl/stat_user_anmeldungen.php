...
$range = "day";
$arGroup = array();
$arWhere = array("STAT = 1 AND IS_VIRTUAL = 0");
$timeStart = time();
$dateFormatPhp = "Y-m-d";
...
if (array_key_exists("FK_USERGROUP", $_REQUEST)) {
    $arWhere[] = "FK_USERGROUP=".$_GET['FK_USERGROUP'];
    $tpl_content->addvar("params", "FK_USERGROUP=".$_REQUEST['FK_USERGROUP']);
}
$colorIndex = 0;
...
switch ($_REQUEST['range']) {
    ...
    case 'day':
        // Per day
        $title = "Anmeldungen pro Tag"; // Zeitraum 30 Tage
        $range = "day";
        $arWhere[] = "STAMP_REG >= DATE_SUB(DATE_FORMAT(CURDATE(),'%Y-%m-%d'), INTERVAL 30 DAY)";
        $arGroup[] = "YEAR(STAMP_REG), MONTH(STAMP_REG), DAY(STAMP_REG)";
        $dateFormatPhp = "Y-m-d";
        $dateFormatMySQL = "%Y-%m-%d";
    ...
    case 'month':
        // Per month
        $title = "Anmeldungen pro Monat"; // Zeitraum 13 Monate
        $range = "month";
        $arWhere[] = "STAMP_REG >= DATE_SUB(DATE_FORMAT(CURDATE(),'%Y-%m-01'), INTERVAL 13 MONTH)";
        $arGroup[] = "YEAR(STAMP_REG), MONTH(STAMP_REG)";
        $dateFormatPhp = "Y-m";
        $dateFormatMySQL = "%Y-%m";
...
if ($cacheStorage->checkContentValidByHash($cacheHash) && !array_key_exists("recache", $_REQUEST)) {
    ...
    $query = "
      SELECT 
        COUNT(*) as anzahl, 
        DATE_FORMAT(STAMP_REG, '".$dateFormatMySQL."') as datum
      FROM user

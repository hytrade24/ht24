...
if ($_REQUEST['FK_AD'] > 0) {
  $arWhere[] = "FK_USER=".$_REQUEST['FK_AD'];
...
switch ($_REQUEST['range']) {
    ...
    case 'day':
        // Per day
        $title = "Anzahl Produkte pro Tag"; // Zeitraum 30 Tage
        $range = "day";
        $arWhere[] = "ID_DATE >= DATE_SUB(DATE_FORMAT(CURDATE(),'%Y-%m-%d'), INTERVAL 30 DAY)";
        $arGroup[] = "YEAR(ID_DATE), MONTH(ID_DATE), DAY(ID_DATE)";
        $dateFormatPhp = "Y-m-d";
        $dateFormatMySQL = "%Y-%m-%d";
    ...
    case 'month':
        // Per month
        $title = "Anzahl Produkte pro Monat"; // Zeitraum 13 Monate
        $range = "month";
        $arWhere[] = "ID_DATE >= DATE_SUB(DATE_FORMAT(CURDATE(),'%Y-%m-01'), INTERVAL 13 MONTH)";
        $arGroup[] = "YEAR(ID_DATE), MONTH(ID_DATE)";
        $dateFormatPhp = "Y-m";
        $dateFormatMySQL = "%Y-%m";
...
if ($cacheStorage->checkContentValidByHash($cacheHash) && !array_key_exists("recache", $_REQUEST)) {
    ...
    $query = "
      SELECT 
        SUM(CREATES) as anzahl, 
        DATE_FORMAT(ID_DATE, '".$dateFormatMySQL."') as datum
      FROM ad_log

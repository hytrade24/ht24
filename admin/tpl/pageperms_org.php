...
#die(ht(dump($nar_deny)));
  ...
  foreach($ar_pages as $s_page)
  {
    ...
    $s_rowref = $s_pageref. rawurlencode($s_page);
    $ar_liste[] = '<tr'. ($i&1 ? ' class="zeile1"' : ''). '>
  <td align="left">'.$s_page.'&nbsp;</td>';
#echo $n_perm_pageperm;
    if ($n_perm_pageperm & PERM_EDIT)
      foreach($ar_roles as $id=>$s_label) $ar_liste[] = '
    <input type="hidden" name="perm['. $id. ']['. $s_page. ']" value="0" />
    <td align="center"><input class="nob" type="checkbox" name="perm['. $id. ']['. $s_page. ']" '
          . (in_array ($s_page, $nar_deny[$id]) ? '' : 'checked '). 'value="1" /></td>';
    elseif ($n_perm_pageperm & PERM_READ)
      foreach($ar_roles as $id=>$s_label) $ar_liste[] = '
    <td align="center">'. (in_array ($s_page, $nar_deny[$id]) ? '-' : '&radic;'). '</td>';
    else
      $ar_liste[] = str_repeat('<td align="center">?</td>', count($ar_roles));
    if ($tmp = $nar_extra[$s_page] && ($n_perm_pageperm & PERM_READ))
    {
      $ar_liste[] = '
  ...
  <td><a href="'. $s_rowref. '" target="eakpop"
    ...
    src="gfx/btn.edit.gif" width="32" height="16" border="0" alt="Ausnahmen bearbeiten" /></a> ('.$s_page['anzahl'].')</td>
</tr>';
    else
      $ar_list[] = '
  <td>&nbsp;</td>';

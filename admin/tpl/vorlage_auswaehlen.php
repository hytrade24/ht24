  ...
  while ($fn = readdir($dp))
    if ('.ttf'==strrchr(strtolower($fn),'.'))
    {
      ...
      else
      {
        ...
        $s = trim($info['name']);
        $s = (strlen($s) ? $s : $fn);
		//echo $s.' <b>'.$info['styles'].'</b> '.$info['minsize'].'<br> ';
        $ar_fonts[$s] = "'$fn'=>'$s'";
        if (count($info))
        {
          $err = array();
...
ksort($ar_fonts);
if (!count($ar_fonts))
  $tpl_content->addvar("arfonts", 'Keine Schriftarten gefunden');
else
  $tpl_content->addvar("schriften", '... Schriften geladen ...');
//write_section("$str_indexfile.php", 'fonts', "\$fonts = array(\n  ". implode(",\n  ", $ar_fonts). "\n);"); 

$fp = @fopen("$str_indexfile.php", 'w');
fputs($fp, "<?php\n \$fonts = array(\n  ". implode(",\n  ", $ar_fonts). "\n);\n?>");
fclose($fp);
...
include ('../cache/fonts.php');
$tpl_content->addvar("fontsc",count($fonts));

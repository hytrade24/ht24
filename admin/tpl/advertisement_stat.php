...
if ($id_ad_user > 0 )
    $where = " AND FK_ADVERTISEMENT_USER=".$id_ad_user;
...
if (!empty($_REQUEST["code"])) {
	die(
		$db->fetch_atom("SELECT CODE FROM `advertisement_user` WHERE ID_ADVERTISEMENT_USER=".$id_ad_user)
	);
}
  ...
  */      
  ...
  $ar_stats = $db->fetch_nar("
	select DATEDIFF(CURDATE(), `STAMP`),sum(`COUNT`) as `COUNT` from `advertisement_stat`
		where `STAMP` >= (CURDATE() - INTERVAL ".$num_days." DAY) ".$where." group by `STAMP`");

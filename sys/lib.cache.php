...
function cache_nav_all($root)
{
    global $db,$langval, $ab_path;
	  ...
	  for($k=0; $k<count($langs); $k++)
	  {
	    ...
	    mysql_data_seek($res, 0);
	    #die(var_dump($res, $nar_lft2id, $nar_kids));
	/** /
	...
	*/
	...
	/**/
	    #die(ht(dump($code)));
	    $fp = fopen($filename = $ab_path.'cache/nav'. $root. '.'.$langs[$k]['ABBR'].'.php', 'w')
	  ...
	  if ('v0'!=$do && 'v1'!=$do)
	  {
	    ...
	    $fp = fopen($fn= $ab_path.'cache/nav'. $root. '.php', 'w')
	      or die("Filesystem error");
	    fwrite($fp, $s_code);
	    fclose($fp);
	    chmod($fn, 0777);
	  ...
	  @unlink ($ab_path."cache/helpnavi.php");
	  @unlink ($ab_path."cache/helpnavi1.php"); //fuer popup
...
function cache_faq()
{
  ...
  foreach ($ar_nav_list as $n => $ar_nav) {
	  for ($l=0; $l<count($langs); $l++)
	  {
		...
		if(!$file_main)
		{
			if (array_key_exists('tpl_content', $GLOBALS) && $GLOBALS['tpl_content'] instanceof Template) {
...
function update_infocache()
{
  ...
  for($n=0; $n<count($ar_lang); $n++)
  {
	  ...
	  $fp = @fopen($ab_path."cache/info.".$ar_lang[$n]['ABBR'].".php", "w");
	  if(!$fp)
	...
	\$ar_byname = array(".implode(",", $ar_byname).");\r\n?>");
	  fclose($fp);
      chmod($ab_path."cache/info.".$ar_lang[$n]['ABBR'].".php", 0777);
...
function cache_rss()
{
  ...
  for($i=0; $i<count($ar_lang); $i++)
  {
	  ...
	  if(!$res['rsrc'])
	    die(ht(dump($res)));
      $tpl_table = new Template($ab_path."skin/".$ar_lang[$i]['ABBR']."/index.rss.htm");
	  ...
	  while($row = mysql_fetch_assoc($res['rsrc']))
	  {
	    ...
	    $tpl_tmp = new Template($ab_path."skin/".$ar_lang[$i]['ABBR']."/index.rss.artikel.htm");
...
function cache_partnerlinks($s_lang='de')
{
  ...
  $tmp_parter = $tmp_paid = array(); //Array initialisieren
	...
	$db->querynow("update partnerlinks set VISIBLE = 0 where PAIDTIL <= now() and VISIBLE=1 and PAID=1");
	...
	$fpl = fopen("../cache/partnerlinks.".$s_lang.".htm", 'w');
	$write_tmp = fwrite($fpl, $tpl_partner->process());
	fclose($fpl);
	chmod("../cache/partnerlinks.".$s_lang.".htm", 0777);

	$fpl = fopen("../cache/paidlinks.".$s_lang.".htm", 'w');
	$write_tmp = fwrite($fpl, $tpl_partner->process());
	fclose($fpl);
	chmod("../cache/paidlinks.".$s_lang.".htm", 0777);

...
class PacketOrderAdTopCustomRecurring extends PacketOrderRecurring {
	...
	*/
	public function activate() {
		parent::activate();
		$result = PacketTargetAdTop::activate($this->database, $this->id, 8);
		$this->cancel();
		return $result;
	}
	...
	public function deactivate() {
		parent::deactivate();
		return PacketTargetAdTop::deactivate($this->database, $this->id, 8);
	}
	...
	public function moveContent($id_packet_order, &$ar_failed = array()) {
		...
		foreach ($ar_usage as $index => $ar_pair) {
			if ($adOrderNew->isAvailable("ad_top_custom", 1)) {
				$adOrderOld->itemRemContent("ad_top_custom", $ar_pair["FK"]);
				$adOrderNew->itemAddContent("ad_top_custom", $ar_pair["FK"]);
			} else {
				if (!is_array($ar_failed["ad_top_custom"])) {
					$ar_failed["ad_top_custom"] = array();
				}
				$ar_failed["ad_top_custom"][] = $ar_pair["FK"];
				$result = false;
			}
		}
		return $result;

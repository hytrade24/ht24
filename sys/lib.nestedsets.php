...
class nestedsets {
  ...
  // don´t set it to true, if you don´t know how to handle locks!
  var $lockTable = false;
 ...
 */
  ...
  /*
    ...
    None
    NOTICE
      if you´ve forgotten to define a query function, the script will die here.
    It will die too, if there´s no function or method with the name $this->queryFunv
  */
...
#     max(zwei.LFT) as P_LFT, min(zwei.RGT) as P_RGT,
  ...
  */
    ...
    $res=$this->query("select * from ".$this->s_table."
      where ROOT=". $this->n_root. "
        and ( RGT = ".($ar_data['LFT']-1)." or LFT =  ".($ar_data['LFT']-1)." )");
  ...
  function nestMoveLeft($id,$ar=array ())
  {
    /*
     Function
      Moves a node and it´s children one position left
     Params
      $this->s_table = Table you are using
    $id = id of node you want to move
    $ar = ar of node to be moved.
...
#die(listtab($GLOBALS['ar_query_log']));
  ...
  function nestMoveRight($id,$ar=array ())
  {
    /*
     Function
    Moves a node and it´s children one position right
     Params
      $this->s_table = Table you are using
    $id = id of node you want to move
    $ar = ar of node to be moved.
  ...
  function nestMoveDown( $id, $ar = array ())
  {
    /*
     Function
    Moves a node and it´s children one position down
     Params
      $this->s_table = Table you are using
    $id = id of node you want to move
    $ar = ar of node to be moved.
...
function pathicon($path)
{
  ...
  return '
  <td width="19" height="17"><img src="'.$ab_baseurl.'gfx/path.'. $path. '.png" width="19" height="17"></td>';
}
  ...
  foreach($data as $i=>$row)
  {
    ...
    if(!empty($add_fields)) {
    	...
    	foreach($add_fields as $id_filter => $ident) {
    		$felder[] = '<td><input type="text" name="" value="" size="30" /></td>';
    	}
    	$tpl_tmp->addvar("add_columns", implode("\n", $felder));
    } else {
    	$tpl_tmp->addvar('add_columns', '<td>x</td>');
    }
    $ar_res[] = $tpl_tmp;
  }

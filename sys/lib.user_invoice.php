...
class UserInvoice {
	...
	public function addAddress($company, $firstname, $lastname, $street, $zip, $city, $fk_country, $phone) {
		global $uid;
		$result = $this->database->querynow("INSERT INTO `user_invoice` (FK_USER, COMPANY, FIRSTNAME, LASTNAME, STREET, ZIP, CITY, FK_COUNTRY, PHONE)
	...
	public function getAddress($id_user_invoice) {
		global $uid, $langval;
		if ($id_user_invoice == 0) {
			...
			return $this->database->fetch1("SELECT u.*, s.V1 as COUNTRY
					...
					LEFT JOIN string s ON s.S_TABLE='country' AND s.FK=c.ID_COUNTRY AND
						s.BF_LANG=if(c.BF_LANG & ".$langval.", ".$langval.", 1 << floor(log(c.BF_LANG+0.5)/log(2)))
					WHERE u.FK_USER=".$uid." AND u.ID_USER_INVOICE=".(int)$id_user_invoice);
		}
	}
	...
	public function removeAddress($id_user_invoice) {
		global $uid;
		$this->database->querynow("DELETE FROM `user_invoice` WHERE ID_USER_INVOICE=".(int)$id_user_invoice." AND FK_USER=".(int)$uid);
		$this->database->querynow("UPDATE `user` SET FK_USER_INVOICE=0 WHERE FK_USER_INVOICE=".(int)$id_user_invoice);
	}

	public function updateAddress($id_user_invoice, $company, $firstname, $lastname, $street, $zip, $city, $fk_country, $phone) {
		global $uid;
		$this->database->querynow("UPDATE `user_invoice` SET
			...
			WHERE ID_USER_INVOICE=".(int)$id_user_invoice." AND FK_USER=".(int)$uid);
	}

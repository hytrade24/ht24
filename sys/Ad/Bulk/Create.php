...
class Ad_Bulk_Create {
    ...
    public static function GetTables(ebiz_db $db, $parentId) {
        $arTablesAvailable = $db->fetch_col("
            SELECT DISTINCT KAT_TABLE FROM `kat` WHERE ROOT=1 AND PARENT=".(int)$parentId." ORDER BY LFT ASC");
        $arTablesUsed = array();
        foreach ($arTablesAvailable as $tableIndex => $tableName) {
            $hdbCount = $db->fetch_atom("
                SELECT COUNT(*) 
                FROM `hdb_table_".mysql_real_escape_string($tableName)."` p
                LEFT JOIN `manufacturers` m ON m.ID_MAN=p.FK_MAN
                WHERE p.CONFIRMED=1 AND m.CONFIRMED=1");
            if ($hdbCount > 0) {
                $arTablesUsed[] = "'".mysql_real_escape_string($tableName)."'";
            }
        }
    ...
    public static function GetProducts(ebiz_db $db, $tableId, $manufacturerId, $userId = null, $page = 1, $perpage = -1, $search = null, &$count = null) {
        $tableName = $db->fetch_atom("
            SELECT T_NAME FROM `table_def` WHERE ID_TABLE_DEF=".(int)$tableId);
        if (empty($tableName)) {
            return array();
        ...
        $searchResult = $hdbManagement->searchProductsByMan($manufacturerId, $page, $perpage, $arProductTypes, $arCategoryIds, $search);
        $count = $searchResult->total;
        $arProducts = $searchResult->results;
    ...
    protected function addStatistic($type, $userId, $value) {
        if (!array_key_exists($type, $this->arStatistics)) {
            $this->arStatistics[$type] = array();
        }
        if (!array_key_exists($userId, $this->arStatistics[$type])) {
            $this->arStatistics[$type][$userId] = $value;
        } else {
            $this->arStatistics[$type][$userId] += $value;
        }
    }
    ...
    protected function getStatistic($type, $userId = null, $default = 0) {
        if (array_key_exists($type, $this->arStatistics)) {
            if ($userId === null) {
                return $this->arStatistics[$type];
            } else {
                if (array_key_exists($userId, $this->arStatistics[$type])) {
                    return $this->arStatistics[$type][$userId];
                } else {
                    return $default;
                }
            }
    ...
    public function addArticle($tableName, $articleData, $userId, $articleId) {
        ...
        $arArticle["DELETED"] = 0;
        $arArticle["AD_CLICKS"] = 0;
        $arArticle["AD_LIKES"] = 0;
        $arArticle["B_TOP"] = 0;
        $arArticle["B_TOP_LIST"] = 0;
        $arArticle["B_PSEUDOPREIS_DISCOUNT"] = 0;
        $arArticle["MOQ"] = 1;
        $arArticle["TRADE"] = 0;
        $arArticle["AUTOBUY"] = 0;
        $arArticle["AUTOCONFIRM"] = 0;
        $arArticle["AFFILIATE"] = 0;
		// Moderate ads?
        ...
        if (!array_key_exists("LATITUDE", $arArticle) || ($arArticle["LATITUDE"] === null)) {
            $arArticle["LATITUDE"] = 0;
        }
        if (!array_key_exists("LONGITUDE", $arArticle) || ($arArticle["LONGITUDE"] === null)) {
            $arArticle["LONGITUDE"] = 0;
        }
        ...
        // Add to statistics
        $this->addStatistic("articlesNew", $userId, 1);
    }
    ...
    public function finish() {
        ...
        $articlesNew = $this->getStatistic("articlesNew");
        foreach ($articlesNew as $userId => $articleCount) {
            $this->db->querynow("INSERT INTO `ad_log` (`ID_DATE`, `FK_USER`,`CREATES`) VALUES ('".date('Y-m-d')."',".(int)$userId.", ".(int)$articleCount.") ON DUPLICATE KEY UPDATE CREATES=CREATES+".(int)$articleCount);
            $this->db->querynow("INSERT INTO `usercontent` (`FK_USER`, `ADS_USED`) VALUES (".(int)$userId.",".(int)$articleCount.") ON DUPLICATE KEY UPDATE ADS_USED=ADS_USED+".(int)$articleCount);
        }
    }

...
class securimage {
  ...
  var $ttf_file = "../admin/fonts/elephant.ttf";
	//admin/fonts/
  ...
  var $data_directory1 = "../cache/img";
	var $data_directory2 = "cache/img";
  ...
  var $hash_salt = "ksjsdudf938jwd7qw783ej";
  ...
  function saveData()
  {
    ...
    $fp = fopen($this->data_directory1 . "/" . $filename, "w+");
  ...
  function validate()
  {
    ...
    $enced_code = trim(@file_get_contents($this->data_directory2 . "/" . $filename));
    
    $check = md5($this->hash_salt . strtolower($this->code_entered));
		
		//die ('filename: '.$filename.'<br>enced_code: '.$enced_code.'<br>check: '.$check);

    if($check == $enced_code) {
      $this->correct_code = TRUE;
      @unlink($this->data_directory2 . "/" . $filename);
    } else {

...
*/
  function ebiz_db($str_dbname, $str_host=NULL, $str_user=NULL, $str_pass=NULL, $newLink = false) {
      ...
      $this->rsrc_db = @mysql_connect($str_host, $str_user, $str_pass, $newLink, 128);
      $success = true;
      ...
      if (!$this->rsrc_db) {
          ...
          if (!mysql_select_db($str_dbname, $this->rsrc_db)) {
              $success = false;
              $sendMail = true;
              if (file_exists($lockFile) && ((time() - filemtime($lockFile)) < 1800)) {
                  // Only send error by email once every 30 minutes
...
#echo "$str_sql<br />";
    ...
    */
    ...
    function querynow($str_sql, $b_die=false, $cursorScroll = false, $bigQuery = false) {
        $eventQuery = false;
        if (array_key_exists('ebizRecordLoadtimeDatabase', $_COOKIE)) {
            $arBacktrace = array();
            foreach (debug_backtrace(0) as $btIndex => $btDetails) {
                unset($btDetails["args"]);
                $arBacktrace[] = $btDetails;
            }
            $eventQuery = Tools_LoadtimeStatistic::getInstance()->createEvent("DB", "MySQL-Query", array("query" => $str_sql, "result" => false, "backtrace" => serialize($arBacktrace)));
        }
        if ($bigQuery) {
            $ret = array ('str_query'=>'Disabled by "bigQuery"-Option.');
            if (version_compare(phpversion(), "7.0.0", ">=")) {
                $ret['rsrc'] = new Legacy_MySQLQuery($str_sql, $cursorScroll, [], $this->rsrc_db);
                if (!$ret['rsrc']->execute()) {
                    $ret['int_result'] = $ret['rsrc']->get_error_code();
                    $ret['str_error'] = implode("\n", $ret['rsrc']->get_error_info());
                    $ret['rsrc'] = false;
                }
            } else {
                $ret['rsrc'] = mysql_query($str_sql, $this->rsrc_db);
            }
            if ($ret['rsrc']) {
                // success
                $ret['int_result'] = true;
                $ret['str_error'] = '';
                $ret['str_command'] = 'Disabled by "bigQuery"-Option.';
            } else {
                // fail
                $ret['int_result'] = mysql_errno($this->rsrc_db);
                $ret['str_error'] = mysql_error($this->rsrc_db);
            }
        } else {
            $tmp = $this->q_queries;
        ...
        if ($eventQuery !== false) {
            $eventQuery->updateDetails(array("result" => [
                "int_result" => $ret["int_result"], "flt_runtime" => $ret["flt_runtime"]
            ]));
            $eventQuery->finish();
        }
...
* Queries ausführen
*
* arbeitet die Query-Queue ab und liefert eine Liste von assoziativen Arrays mit den Ergebnissen zurück.
* Ist $fl_ignoreerrors TRUE, wird ohne Rücksicht auf Fehlermeldungen durchgearbeitet;
* sonst wird bei einem Fehler abgebrochen; die nicht ausgeführten Queries erhalten den Status 'not executed'

* @param boole fl_ignoreerrors = false

* @return string str_query Query-String
* @return mixed rsrc: 'not executed': false; sonst Rückgabe von mysql_query()
...
* @global $lastresult enthält das Ergebnis der letzten ausgeführten Query
*/
...
#echo ht($str_sql), '<br />';
      ...
      {
#echo ht(dump($row));
        $str_command = substr(strtolower(trim($str_sql)), 0,
...
#echo 'update';
          case 'delete':
#echo 'delete';
            $row['int_result'] = mysql_affected_rows();
            break;
...
* @param uint $int_start=false hilfreich beim Blättern: Nummer des ersten zu liefernden Datensatzes
* @param uint $int_count=false max. Anzahl der zurückzuliefernden Zeilen
* @return array (assoc)
*/
  /*
...
#echo ht(dump($res));
    ...
    $str_keycol = preg_replace('/^`?([^`]*)`?$/', '$1', $str_keycol);
    ...
    for ($i=0; (false===$int_count || $i<$int_count)
      && ($row = mysql_fetch_array ($rsrc, $int_result_type)); $i++)
      if ($str_keycol)
...
*/
  function update_convert($s_fieldname, &$data, &$def)
  {
    ...
    // anderes NULL
        ...
        return 'NULL';
    // Zahlen
    if (is_numeric($val) || (!is_array($val) && preg_match('/^(\d+|\d*\.\d+|\d+\.\d*)$/', $val)))
      return "'".$val."'";
...
/**
* Datensatz erstellen oder aktualisieren - nur für Tabellen, die den DB-Konventionen folgen!
*
* Ermittelt automatisch den Namen der Spalte für den Primary Key,
* unterscheidet, ob insert und update nötig ist, escaped die Daten.
* Sprachabhängige Inhalte werden zu der aktuellen Sprache gespeichert.
*
...
*/
    function update($str_tablename, $data, $ignoreErrors = false)
        /*
        ...
        */
    {
        ...
        /*
         * Sprache kann als $data[BF_LANG] mit übergeben werden. Wenn nicht, wird
         * wie bisher auf $langval zurückgegriffen.
         *
         * @author Danny Rosifka
         * @date 2011-10-12
         */
        if (isset($data[$s_langfield])) {
            $langval = (int)$data[$s_langfield];
            unset($data[$s_langfield]);
        }
...
#die("update `$str_tablename` set ". implode(',', $set). " where `$str_pkname`=$id");
            ...
            if (!empty($set)) {
                $dbres = $this->querynow($str_lastquery = "
                    update `$str_tablename` 
                    set ".implode(',', $set)." 
                    where `$str_pkname`=$id");
    #die(ht(dump($dbres)));
                $fl_insert = !$dbres['rsrc'];
                if (!$ignoreErrors && ($dbres['rsrc'] === false)) {
    ...
    #die(ht(dump($dbres)));
        ...
        if ($id && $b_setlang) {
            ...
            if (!$fl_insert) {
                $fl_insert = !(int)$this->fetch_atom("
                    select count(*) from $s_langtbl
            ...
            if ($fl_insert) {
                ...
                foreach ($ar_strfields as $dummy => $k) if (!is_null($data[$k])) {
                    $keys[] = $k;
                    $vals[] = "'" . mysql_escape_string($data[$k]) . "'";
                }
                $this->querynow("
                    insert into `$s_langtbl`
                      (".implode(',', $keys).")
                    values
                      (".implode(',', $vals).")");
            } else {
                $set = array();
                foreach ($ar_strfields as $dummy => $k) {
                    $set[] = $k.'='.(is_null($data[$k]) ? 'NULL' : "'" . mysql_escape_string($data[$k]) . "'");
                }
...
/**
* Datensatz erstellen oder aktualisieren - für Tabellen mit mehrspaltigem Primary Key
*
* Falls kein Primary, aber ein Unique Key existiert, müssen die Spaltennamen
...
*
* lädt die Tabellendefinition und erstellt einen Datensatz mit den Default-Werten
*
* @param string $str_tablename Tabellenname
* @global uint $langval Bitwert der aktuellen Sprache
...
*
* Lädt den ersten Datensatz, der von der Query geliefert wird
*
...
/**
* eindimensionales assoziatives Array (Schlüssel => Wert) laden
*
* liefert ein assoziatives Array, die ersten beiden selektierten Spalten liefern Schlüssel bzw. Werte.
* Insbesondere nützlich für Dropdowns u.ä.
*
* @param string $str_query Query-String
...
/**
* Query für mehrsprachige Datensätze erstellen
*
* ermittelt, ob die angegebene Tabelle mehrsprachige Daten enthält und baut eine entsprechende Query (gegebenenfalls mit join) auf
...
* @return assoc (feld1=>feld2)
*/
  function lang_select($s_table, $s_fields = '*', $b_force_langjoin=false, $b_get_langid=false)
  {
    global $langval;
    if (SESSION && $_SESSION['lang_select'] && $_SESSION['lang_select'][$langval]
    ...
    else
    {
      ...
      if (preg_match('/^BF_LANG(_[\w_]+)?$/m', implode("\n", array_keys($this->getdef($s_table))), $match))
      {
        ...
        if ($b_get_langid)
          $s_tfields .= ',  floor(log(2,s.BF_LANG)) as FK_LANG';
...
/**/
    ...
    return ($lastresult['int_result'] ? $bf_mask & (int)$tmp : $bf_mask);

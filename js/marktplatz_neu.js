    ...
    if (!b_done) {
		...
		*/
        ...
        if (id_kat > 0) {
          ...
          if (confirm('M�chten Sie "'+name_current+'" als Kategorie w�hlen?')) {
...
function UpdateList_Manufacturer(manufacturers) {
	...
	if (man_exact_name.length > 1) {
		...
		if (typeof(manufacturers) == "object") {
			...
			man_entry = document.createElement("li");
			man_entry.innerHTML = "<h3>Bitte warten, Datenbank wird durchsucht...</h3>";
			liste.appendChild(man_entry);
		}
		...
		if (man_count > 0) {    
			...
			man_entry_desc.innerHTML = "<div class='div'><h3>Hersteller aus der Datenbank ausw&auml;hlen:</h3>"+
			    "Klicken Sie auf einen Hersteller aus der folgenden Liste, um diesen auszuw&auml;hlen.</div>";
			...
			man_entry_desc = document.createElement("li");
			man_entry_desc.innerHTML = "<div class='div'><h3>Ihre Eingabe &uuml;bernehmen:</h3>"+
				"Klicken Sie auf den folgenden, von ihnen eingegebenen, Hersteller um diesen auszuw&auml;hlen.</div>";
			liste.appendChild(man_entry_desc);
            liste.appendChild(man_entry);
		}
	} else {
		man_entry = document.createElement("li");
		man_entry.innerHTML = "<h3>Bitte Hersteller eingeben!</h3>"+
             "<a title='Ohne Auswahl eines Herstellers fortfahren' href='javascript:UpdateSelection_Manufacturer(179,\"Unbekannter%20Hersteller\");'>"+
 			...
 			if (typeof(products) == "object") {
                ...
                var product_entry_desc;
 				if (products.length > 0) {
 			        ...
 			        product_entry_desc.innerHTML = "<div class='div'><h3>Produkt aus der Datenbank ausw&auml;hlen:</h3>"+
                       "Klicken Sie auf ein Produkt aus der folgenden Liste, um dieses auszuw&auml;hlen.</div>";
 			        liste.appendChild(product_entry_desc);
 				} else {
 				   	product_entry_desc = document.createElement("li");
 				       product_entry_desc.innerHTML = "<div class='div'><h3>Ihre Eingabe &uuml;bernehmen:</h3>"+
 				           "Klicken Sie auf das folgende, von ihnen eingegebenen, Produkt um dieses auszuw&auml;hlen.</div>";
 				       liste.appendChild(product_entry_desc);
 				...
 				// Wird noch gesucht
 				product_entry = document.createElement("li");
 				product_entry.innerHTML = "<h3>Bitte warten, Datenbank wird durchsucht...</h3>";
 				liste.appendChild(product_entry);
 			}
       } else {
 			product_entry = document.createElement("li");
 			product_entry.innerHTML = "<h3>Bitte Suchbegriff / Produktnamen eingeben!</h3>";
 			liste.appendChild(product_entry);
	}
...
function Pager_Redraw(div_id) {
	...
	if (pager.maxpage > 1) {
		...
		description.innerHTML = (pager.maxcount+" Ergebnisse gefunden (Seite "+pager.curpage+" von "+pager.maxpage+")");
		...
		success: 	function(obj) {
	        if (!obj.valid) {
	        	...
	        	if (jQuery('#' + name + '_ERROR').length > 0) {
		            ...
		            jQuery('#' + name + '_ERROR')[0].title = "Eingabe korrekt oder optional.";

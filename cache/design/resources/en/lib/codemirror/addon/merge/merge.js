...
(function(mod) {
  ...
  DiffView.prototype = {
    ...
    init: function(pane, orig, options) {
      ...
      this.trimLines = (typeof options.trimLines !== "undefined" ? options.trimLines !== false : false);
      this.timeout = options.timeout;

      this.diff = getDiff(asString(orig), asString(options.value), this.trimLines, this.timeout);
  ...
  function ensureDiff(dv) {
    if (dv.diffOutOfDate) {
      dv.diff = getDiff(dv.orig.getValue(), dv.edit.getValue(), dv.trimLines, dv.timeout);
  ...
  function getDiff(a, b, trimLines, timeout) {
    var diff;
    dmp.Diff_Timeout = (typeof timeout === "undefined" ? 1 : parseFloat(timeout));
    if (trimLines) {
        var linesA = a.split("\n");
        var linesB = b.split("\n");
        var x;
        for (x = 0; x < linesA.length; x++) { linesA[x] = linesA[x].replace(/(^\s+|\s+$)/g, ""); }
        for (x = 0; x < linesB.length; x++) { linesB[x] = linesB[x].replace(/(^\s+|\s+$)/g, ""); }
        diff = dmp.diff_main(linesA.join("\n"), linesB.join("\n"));
    } else {
        diff = dmp.diff_lineMode_(a, b);
    }

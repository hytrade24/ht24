...
#Anzahl der Useranmeldungen
function stats_usercounter($height="200", $width="400")
{
		global $db;
		$max=0;
        $month_back = 13;
...
for ($i = 0; $i < $month_back; $i++) {
		...
		$ar = $db->fetch_table("select count(*) as anzahl,concat(YEAR(STAMP_REG),'-',MONTH(STAMP_REG)) as STAMP from user
where STAMP_REG >= DATE_SUB(DATE_FORMAT(CURDATE(),'%Y-%m-01'),INTERVAL ".$month_back." MONTH)
group by YEAR(STAMP_REG),MONTH(STAMP_REG) order by STAMP_REG DESC");
 ...
 if(!empty($ar))
  ...
  for ($i = 0; $i < count($ar); $i++) {
        ...
        if ($max < $ar[$i]['anzahl']) $max = $ar[$i]['anzahl'];

        $data[$ar[$i]['STAMP']]=$ar[$i]['anzahl'];
        $label[$ar[$i]['STAMP']]=$ar[$i]['STAMP'];
  }


		$g = new graph();
        $g->set_data(array_reverse($data));
         $g->title( 'Useranmeldungen ('.date('d.m.y h:i').')', '{font-size: 10px; color: #736AFF}' );


	#	$g->line( 2, '0x9933CC', 'User  ', 10 );
		$g->line_dot( 3, 5, '0xCC3399', 'User ', 10);
	#	$g->line( 1, '0xFF0000', 'Visits  ', 10 );

		$g->set_x_labels( array_reverse($label) );
		$g->set_x_label_style( 10, '0x000000', 2, 1 );
		$g->set_y_max( $max );
		$g->set_y_legend( 'Anmeldungen ', 10, '#736AFF' );
		$g->y_label_steps( 5 );
...
#Bday Anzahl der user
function bdays_getdata2 ($height="200", $width="800")
{
	global $db;
    $data = $label = array();

	$nar_data = $db->fetch_table("select TIMESTAMPDIFF(YEAR,GEBDAT,CURDATE()) as age , count(GEBDAT) as anzahl FROM user group by age",'age'); // Daten aus Tabelle holen

	$maxanzahl = $sumanzahl = 0;

	for ($i = 10; $i < 60; $i++) {

		if (!array_key_exists($i, $nar_data)) {
			$nar_data[$i]['anzahl']='0';
			$nar_data[$i]['age']=$i;
            $data[$i]=0;
            $label[$i]=$i;
         ...
         {
            $data[$i]=$nar_data[$i]['anzahl'];
            $label[$i]=$nar_data[$i]['age'];
         }
		...
		$g = new graph();
        $g->set_data(array_reverse($data));
     $g->title( 'Altersverteilung ('.date('d.m.y h:i').')', '{font-size: 10px; color: #736AFF}' );


	#	$g->line( 2, '0x9933CC', 'User  ', 10 );
		$g->line_dot( 3, 5, '0x9933CC', 'Alter ', 10);
	#	$g->line( 1, '0xFF0000', 'Visits  ', 10 );

		$g->set_x_labels( array_reverse($label) );
		$g->set_x_label_style( 10, '0x000000', 2, 1 );
		$g->set_y_max( $maxanzahl );
		$g->set_y_legend( 'Anzahl ', 10, '#736AFF' );
		$g->y_label_steps( 5 );
...
function user_write_views($userid,$cache)
{
		global $db;
		$dayrange=32;
		$max = 0;

		$stamp = @filemtime($GLOBALS['nar_systemsettings']['SITE']['USER_PATH'].$cache.'/'.$userid."/stats_user_view.php");
  		if(!$stamp || $stamp < (time()-3600))
  		{
				...
				for ($i = 0; $i < $dayrange; $i++) {
					$data_stats[date("Y-m-d",mktime(0, 0, 0, date("m"), date("d")-$i, date('Y')))]="0";
					$label[date("Y-m-d",mktime(0, 0, 0, date("m"), date("d")-$i, date('Y')))]=date("Y-m-d",mktime(0, 0, 0, date("m"), date("d")-$i, date('Y')));
				}
				//userdaten lesen
				$ar = $db->fetch_table("select sv.STAMP,sum(views) as views
				from user_views sv
				where  sv.STAMP >= DATE_SUB(DATE_FORMAT(CURDATE(),'%Y-%m-%d'),INTERVAL ".$dayrange." DAY) and FK_USER = ".$userid."
				group by sv.STAMP order by sv.STAMP DESC");

				 if(!empty($ar))
					{
				  for ($i = 0; $i < count($ar); $i++) {

					if ($max < $ar[$i]['views']) $max = $ar[$i]['views'];

					$data_stats[$ar[$i]['STAMP']]=$ar[$i]['views'];
					$label[$ar[$i]['STAMP']]=$ar[$i]['STAMP'];
				  }
				  }

				  	  $s_code = '<?'. 'php $data_stats = '. php_dump($data_stats, 0). ';';
	  $s_code .= ' $max = '.$max. ';';
	  $s_code.= ' $label = '. php_dump($label, 0). ';?'.'>';
      $fp = fopen($GLOBALS['nar_systemsettings']['SITE']['USER_PATH'].$cache.'/'.$userid."/stats_user_view.php", 'w');
       fputs($fp, $s_code);
       fclose ($fp);
		...
		{
		  include ($GLOBALS['nar_systemsettings']['SITE']['USER_PATH'].$cache.'/'.$userid."/stats_user_view.php");
		  $cached='!';
		}
		$g = new graph();
		$g->set_data( $data_stats );
		$g->line( 2, '0x9933CC', 'Profile views', 10 );
		$g->set_x_labels( $label );
		$g->set_x_label_style( 10, '0x000000', 2, 2 );
		$g->set_y_max( $max );
		$g->set_y_legend( 'VIEWS '.$cached.' '.date("H:i:s",$stamp), 10, '#736AFF' );
		$g->y_label_steps( 5 );
		$g->set_width( '100%' );
		$g->set_height( '200' );
		$g->set_output_type('js');
...
function script_write_ratigs($ID_SCRIPT)
{
		global $db;
		$range=13;
		$max = 0;

		$stamp = @filemtime($GLOBALS['nar_systemsettings']['SITE']['USER_PATH'].'scripts/stats_script_rating.'.$ID_SCRIPT.'.php');
  		if(!$stamp || $stamp < (time()-3600))
  		{
				...
				// Array auff�llen
				for ($i = 0; $i < $range; $i++) {
					$data_stats[date("Y-n",mktime(0, 0, 0, date("m")-$i, '01', date('Y')))]="0";
					$label[date("Y-n",mktime(0, 0, 0, date("m")-$i, '01', date('Y')))]=date("Y-n",mktime(0, 0, 0, date("m")-$i, '01', date('Y')));
				}
				//userdaten lesen
				$ar = $db->fetch_table("select sum(RATING)/count(FK)as views ,concat(YEAR(sv.STAMP),'-',MONTH(sv.STAMP)) as STAMP
from rating_script sv
where FK = ".$ID_SCRIPT." and sv.STAMP >= DATE_SUB(DATE_FORMAT(CURDATE(),'%Y-%m-01'),INTERVAL ".$range." MONTH)
group by YEAR(STAMP),MONTH(STAMP) order by sv.STAMP DESC;");
	 ...
	 $s_code = '<?'. 'php $data_stats = '. php_dump($data_stats, 0). ';';
	  $s_code .= ' $max = '.$max. ';';
	  $s_code.= ' $label = '. php_dump($label, 0). ';?'.'>';
      $fp = fopen($GLOBALS['nar_systemsettings']['SITE']['USER_PATH'].'scripts/stats_script_rating.'.$ID_SCRIPT.'.php', 'w');
       fputs($fp, $s_code);
       fclose ($fp);
		...
		{
		  include ($GLOBALS['nar_systemsettings']['SITE']['USER_PATH'].'scripts/stats_script_rating.'.$ID_SCRIPT.'.php');
		  $cached='!';
		}
		$g = new graph();
		$g->set_data( $data_stats );
		$g->line( 2, '0x9933CC', 'durchschnittliche Bewertung ', 10 );
		$g->set_x_labels( $label );
		$g->set_x_label_style( 10, '0x000000', 2, 1 );
		$g->set_y_max( 5 );
		$g->set_y_legend( 'RATING '.$cached.' '.date("H:i:s",$stamp), 10, '#736AFF' );
		$g->y_label_steps( 5 );
	...
	*/
	...
	function render()
	{
		...
		if($this->output_type == 'js')
		{
			...
			$tmp[] = '<script type="text/javascript" src="'.$tpl_content->tpl_uri_baseurl("/lib/swfobject.js").'"></script>';
			$tmp[] = '<script type="text/javascript">';
			$tmp[] = 'var so = new SWFObject("'.$tpl_content->tpl_uri_baseurl("/open-flash-chart.swf").'?t='.date("m").'", "ofc", "'. $this->width . '", "' . $this->height . '", "9", "#FFFFFF");';

		...
		success: 	function(obj) {
	        ...
	        if (!obj.valid) {
	        	...
	        	if (jQuery('#' + name + '_INPUT').length > 0) {
	        		...
	        		jQuery('#' + name + '_INPUT .help-inline').html(icon_success+'[[ translation : marketplace : ad.create.input.valid :: Eingabe korrekt oder optional. ]]');
	        	}
...
function UploadFile() {
	...
	jQuery.ajaxFileUpload({
		...
		error: function() {
			alert("[[ translation : marketplace : ad.create.error.upload.failed :: Upload failed! ]]");
		}
	});
}
	...
	jQuery.ajax({
		...
		success: function(obj) {
			if (obj.success) {
				...
				alert("[[ translation : marketplace : ad.create.error.confirm :: Fehler beim bestätigen der Anzeige! ]]");
				jQuery("#modal_confirm").modal("hide");
			}
		}
	});

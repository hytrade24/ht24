...
function ShowStep(stepIndex, ajax, options) {
    ...
    // Show loading notice
    ShowLoadingNotice('[[ translation : marketplace : ad.create.step.loading :: Lade Schritt... ]]');
    // Replace undefined parameters by defaults
    if (typeof ajax == "undefined") {
        ajax = true;
    }
    ...
    if (options.save) {
        ...
        var question = '[[ translation : marketplace : ad.create.fields.save :: Möchten Sie die vorgenommenen Änderungen speichern? ]]';
...
function SubmitStepRaw(postData, options, form) {
    ...
    // Show loading notice
    ShowLoadingNotice('[[ translation : marketplace : ad.create.step.saving :: Eingaben werden gespeichert... ]]');
    // Replace undefined parameters by defaults
    if (typeof options == "undefined") {
        options = {};
    }
    ...
    step_request = jQuery.post(url, postData, function(result) {
        ...
        if (result.success) {
            ...
            if (errorText.length > 0) {
                alert( "[[ translation : marketplace : ad.create.error.alert :: Fehler beim Speichern: ]]\n" + errorText.join("\n") );
            }
        }
    });
}
    ...
    return jQuery.ajax({
        ...
        success: 	function(obj) {
            if (!obj.valid) {
                ...
                if (jQuery('#' + name + '_INPUT').length > 0) {
                    var icon_success = '<img src="'+ebiz_trader_baseurl+'bilder/ok.png" class="js-tooltip" data-toggle="tooltip" title="[[ translation : marketplace : ad.create.input.valid :: Eingabe korrekt oder optional. ]]"  />&nbsp;';

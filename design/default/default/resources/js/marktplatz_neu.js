    ...
    if (!b_done) {
		...
		*/
        ...
        if (id_kat > 0) {
          ...
          if (confirm('[[ translation : marketplace : ad.create.category.confirm : NAME="' + name_current + '" : Möchten Sie "{NAME}" als Kategorie wählen? ]]')) {
...
function UpdateList_Manufacturer(manufacturers) {
	...
	if (man_exact_name.length > 1) {
		...
		if (typeof(manufacturers) == "object") {
			...
			man_entry = document.createElement("li");
			man_entry.innerHTML = "<h3>[[ translation : marketplace : manufacturers.loading :: Bitte warten, Datenbank wird durchsucht... ]]</h3>";
			liste.appendChild(man_entry);
		}
		...
		if (man_count > 0) {    
			...
			man_entry_desc.innerHTML = "<div class='div'><h3>[[ translation : marketplace : manufacturer.select.from.db :: Hersteller aus der Datenbank auswählen ]]:</h3>"+
			    "[[ translation : marketplace : manufacturer.select.from.db.lead :: Klicken Sie auf einen Hersteller aus der folgenden Liste, um diesen auszuwählen. ]]</div>";
			...
			man_entry_desc = document.createElement("li");
			man_entry_desc.innerHTML = "<div class='div'><h3>[[ translation : marketplace : manufacturer.use.input :: Ihre Eingabe übernehmen ]]:</h3>"+
				"[[ translation : marketplace : manufacturer.use.input.lead :: Klicken Sie auf den folgenden, von ihnen eingegebenen, Hersteller um diesen auszuwählen. ]]</div>";
			liste.appendChild(man_entry_desc);
            liste.appendChild(man_entry);
		}
	} else {
		man_entry = document.createElement("li");
		man_entry.innerHTML = "<h3>[[ translation : marketplace : manufacturers.error.missing :: Bitte Hersteller eingeben! ]]</h3>"+
             "<a title='[[ translation : marketplace : manufacturer.skip :: Ohne Auswahl eines Herstellers fortfahren ]]' href='javascript:UpdateSelection_Manufacturer(179,\"Unbekannter%20Hersteller\");'>"+
 			...
 			if (typeof(products) == "object") {
                ...
                var product_entry_desc;
 				if (products.length > 0) {
 			        ...
 			        product_entry_desc.innerHTML = "<div class='div'><h3>[[ translation : marketplace : product.use.from.db :: Produkt aus der Datenbank auswählen ]]:</h3>"+
                       "[[ translation : marketplace : product.use.from.db.lead :: Klicken Sie auf ein Produkt aus der folgenden Liste, um dieses auszuwählen. ]]</div>";
 			        liste.appendChild(product_entry_desc);
 				} else {
 				   	product_entry_desc = document.createElement("li");
 				       product_entry_desc.innerHTML = "<div class='div'><h3>[[ translation : marketplace : product.use.input :: Ihre Eingabe übernehmen ]]:</h3>"+
 				           "[[ translation : marketplace : product.use.input.lead :: Klicken Sie auf das folgende, von ihnen eingegebenen, Produkt um dieses auszuwählen. ]]</div>";
 				       liste.appendChild(product_entry_desc);
 				...
 				// Wird noch gesucht
 				product_entry = document.createElement("li");
 				product_entry.innerHTML = "<h3>[[ translation : marketplace : products.loading :: Bitte warten, Datenbank wird durchsucht... ]]</h3>";
 				liste.appendChild(product_entry);
 			}
       } else {
 			product_entry = document.createElement("li");
 			product_entry.innerHTML = "<h3>[[ translation : marketplace : products.empty :: Bitte Suchbegriff / Produktnamen eingeben! ]]</h3>";
 			liste.appendChild(product_entry);
	}
...
function Pager_Redraw(div_id) {
	...
	if (pager.maxpage > 1) {
		...
		description.innerHTML = "[[ translation : marketplace : pager.counter.text : COUNT='"+pager.maxcount+"',CURPAGE='"+pager.curpage+"',MAXPAGE='"+pager.maxpage+"' :: {COUNT} Ergebnisse gefunden (Seite {CURPAGE} von {MAXPAGE}) ]]";
		...
		success: 	function(obj) {
	        if (!obj.valid) {
	        	...
	        	if (jQuery('#' + name + '_ERROR').length > 0) {
		            ...
		            jQuery('#' + name + '_ERROR')[0].title = "[[ translation : marketplace : ad.create.input.valid :: Eingabe korrekt oder optional. ]]";
...
function showMap(self, container){
    if (container.css("display") == 'none') {
        jQuery(self).html('[[ translation : marketplace : ad.create.map.hide :: Karte ausblenden ]]');
        container.show();
        if (!map_loaded) {
        	map_loaded = true;
    	...
    	jQuery(self).html('[[ translation : marketplace : ad.create.map.show :: Karte anzeigen ]]');
        container.hide();
    }
}
